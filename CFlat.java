...
public class CFlat {
	...
	/**
	 * CFlat constructor for client programs and testers. Note that
	 * users MUST invoke {@link setInfile} and {@link setOutfile}
	 */
	public CFlat(){}
	
	/**
	 * If we are directly invoking CFlat from the command line, this
	...
	public int process(){
		...
		if (ErrMsg.getErr()) {  
			return CFlat.RESULT_SYNTAX_ERROR;
		}
		...
		if (ErrMsg.getErr()) {
			return CFlat.RESULT_NAME_ERROR;
		}
		...
		if (ErrMsg.getErr()) {
			...
			return CFlat.RESULT_TYPE_ERROR;
		}

		Codegen.p = new PrintWriter(outFile);
		astRoot.codeGen();		
		Codegen.p.close();
		return CFlat.RESULT_CORRECT;
	}
	...
	public void run() {
		...
		case RESULT_NAME_ERROR:
			pukeAndDie("Name error", resultCode);
